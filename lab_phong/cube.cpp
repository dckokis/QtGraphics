...
Cube::Cube(const GLfloat edgeLen, const GLint gridStep) : m_edgeLen(edgeLen), m_gridStep(gridStep), m_vertexBuf(QOpenGLBuffer::VertexBuffer), m_indexBuf(QOpenGLBuffer::IndexBuffer) {}


void Cube::initialize(){
    initializeOpenGLFunctions();
    m_vertexBuf.create(); m_indexBuf.create();
...
QColor Cube::getColor() const {
    return m_color;
}

void Cube::setFace(const QMatrix4x4& matrix) {
    ...
    for (size_t i = 0; i < m_gridStep; ++i) {
    	for (size_t j = 0; j < m_gridStep; ++j) {
		    ...
		    m_indices.push_back(first); m_indices.push_back(second); m_indices.push_back(second + 1);
		    m_indices.push_back(second + 1); m_indices.push_back(first + 1); m_indices.push_back(first);
    	}
    }
}
