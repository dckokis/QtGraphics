...
void main()
{
    ...
    vec3 normal = normalize(vec3(mat4(transpose(inverse(model))) * vec4(normAttr, 1.0f)));

    col = (isAmbient * ambient() + isPoint * point(fragPos, normal) + isDirected * directed(fragPos, normal) + isProjector * projector(fragPos, normal)) * objectColor;
